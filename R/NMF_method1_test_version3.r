#tg_list=NMF_selected_R1
#data_ng=data.matrix0_s
#data_normalized=data_23_s
#max_ES_cut=0.3
#tProp
#NMF_RR=1000
...
NMF_method1_test_version3<-function(tg_list, data_ng, data_normalized,max_ES_cut2=0.1, max_ES_cut=0.3,tProp,NMF_RR=10)
{
		tg_id0 <- tg_list	#wnchang add for pkg
        ...
        while((ES_max>max_ES_cut2)&(length(tg_id0)>0))
        {
                N<-N+1
		    ...
		    print(tg_selected_id)
	          print("1---------------------")
				...
				if(sum(ES_base_c[tg_selected_id]>max_ES_cut))
				{
					tg_ccc<-tg_selected_id[which(ES_base_c[tg_selected_id]>max_ES_cut)]
					tg_id_del<-c(tg_id_del,tg_ccc)
                              ES_base_c[tg_id_del]<-0
					tg_selected_id<-setdiff(tg_selected_id,tg_id_del)
				}
                }
	          print("2---------------------")
		    print(ES_base_c)
		    ...
		    print(tg_selected_id)
	          print("2---------------------")
		    tg_id0<-setdiff(1:length(tg_selected_R4_RR),c(tg_selected_id,tg_id_del))
        }
        return(list(qnmf_result_c,track_N_cor,NMF_self_c1))
}

NMF_method1_test_version3_pkg<-function(tg_list, data_ng, data_normalized,max_ES_cut2=0.1, max_ES_cut=0.3,NMF_RR=10)
{
	print("qnmf part")
		tg_id0 <- tg_list	#wnchang add for pkg
        tg_selected_R4_RR<-tg_list
        ...
        while((ES_max>max_ES_cut2)&(length(tg_id0)>0))
        {
                ...
                names(NMF_list_c)<-names(tg_selected_R4_RR)[tg_selected_id]     #?????
                NMF_self_c1<-Building_NMF_input_no_cancer(tg_data=data.matrix0_s,tg_list=NMF_list_c,tg_list_add=list())
                qnmf_result_c_temp <- run_NMF_r2(NMF_self_c1,RR0=0.8, maxIter=20000)
                U_temp = qnmf_result_c_temp[["U"]]
                V_temp = qnmf_result_c_temp[["V"]]
                ########check correlation between: V and true Proportion
                V_c<-normalize_data2(t(V_temp))
