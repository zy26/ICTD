...
#compute_CompRowspace_NN_uni


#------------------
.onLoad_temp_func3 <- function()
{
	IM_reso_level<-rep(0,length(names(ICTD::IM_id_list)))
	names(IM_reso_level)<-names(ICTD::IM_id_list)
	IM_reso_level[1:12]<-1
	IM_reso_level[13]<-2
	IM_reso_level[14]<-3
	IM_reso_level[15]<-4
	IM_reso_level[16]<-2
	IM_reso_level[17]<-4
	 ...
	 assign("IM_reso_level", IM_reso_level, envir = parent.env(environment()))
}
